...
var addNumber = function (note, number) {
	...
	console.assert(newIndex > -1 && newIndex < scale.length);

	return scale[newIndex];
};

// Displays the note in string format
var displayNote = function (note) {
	if (!note) return null;
	if (!note.accidental) return note.letter;

	if (note.accidental === 'n') {
		return note.letter.toString();
	}
...
var distanceBet = function (letter1, letter2) {
	...
	console.assert(distance > 0 && distance <= scale.length);
...
Note.prototype.transpose = function (int, down) {
	...
	console.assert(isFinite(halfStepOffset));
...
Note.prototype.higherThan = function (note) {
	...
	sameNote =
		sameNote && (!this.octave || !note.octave || this.octave === note.octave);
...
module.exports.random = function (range) {
	...
	var lower = createNote(range[0]);
	var upper = createNote(range[1]);
...
module.exports.displayNote = displayNote;

module.exports.fromValue = function (value) {
	return createNote('C', Math.floor(value / 12) - 1)
		.shift(value % 12)
		.clean();
};
